...
function generateVarsityJVSquads() {
  ...
  try {
    ...
    try {
      ...
      toast('Error: "Tryout Rankings" sheet not found. Please generate tryout rankings first.');
      return;
    }
    ...
    toast('Varsity/JV squad proposals generated successfully! Check the "Varsity/JV - Autogenerated" sheet.');
  } catch (e) {
...
function generateVarsityJVProposal_(tryoutData, settings) {
  ...
  const requiredEvents = [
    ...
    '100 Breast', '100 Fly', '100 Back', '200 IM'
  ];
  ...
  const maleSwimmers = swimmers.filter(row => row[1] && row[1].toString().toUpperCase() === 'M');
  const femaleSwimmers = swimmers.filter(row => row[1] && row[1].toString().toUpperCase() === 'F');
  
  // Calculate rankings for each gender
  const maleRankings = calculateTryoutRankings_(maleSwimmers, eventColumns, 'Male');
  const femaleRankings = calculateTryoutRankings_(femaleSwimmers, eventColumns, 'Female');
...
function createVarsityJVSheet_(maleRankings, femaleRankings, settings) {
  ...
  sheet.getRange(currentRow, 1).setValue(`Varsity/JV Squad Proposal - Generated ${new Date().toLocaleString()}`);
  sheet.getRange(currentRow, 1, 1, 6).setBackground('#2c5aa0').setFontColor('white').setFontWeight('bold');
  currentRow += 2;
  ...
  function writeGenderSquads(rankings, genderLabel, varsitySpots) {
    ...
    // Gender header
    sheet.getRange(currentRow, 1).setValue(`${genderLabel} Squad (${rankings.length} swimmers)`);
    sheet.getRange(currentRow, 1, 1, 6).setBackground('#4a90e2').setFontColor('white').setFontWeight('bold');
    currentRow++;
    ...
    const bubbleEnd = Math.min(rankings.length, varsitySpots + settings.bubbleSize);
    
    // Write Varsity section
    if (varsityCount > 0) {
      sheet.getRange(currentRow, 1).setValue('VARSITY');
      sheet.getRange(currentRow, 1, 1, 6).setBackground('#90EE90').setFontWeight('bold');
      currentRow++;
      
      // Varsity headers
      const headers = ['Rank', 'Name', 'Avg Rank', 'Best Rank', 'Best Event', 'Status'];
      sheet.getRange(currentRow, 1, 1, headers.length).setValues([headers]);
      sheet.getRange(currentRow, 1, 1, headers.length).setFontWeight('bold').setBackground('#e6f3ff');
      currentRow++;
      ...
      for (let i = 0; i < varsityCount; i++) {
        ...
        if (i >= bubbleStart) {
          sheet.getRange(currentRow, 1, 1, 6).setBackground('#FFE4B5'); // Light orange for bubble
        }
        currentRow++;
      }
    }
    ...
    if (rankings.length > varsitySpots) {
      ...
      sheet.getRange(currentRow, 1, 1, 6).setBackground('#FFA500').setFontColor('white').setFontWeight('bold');
      currentRow++;
  ...
  const summaryData = [
    ...
    ['JV Swimmers', Math.max(0, femaleRankings.length - settings.varsitySpotsF), Math.max(0, maleRankings.length - settings.varsitySpotsM)]
  ];
  
  sheet.getRange(currentRow, 1, summaryData.length, 3).setValues(summaryData);
  sheet.getRange(currentRow, 1, 1, 3).setFontWeight('bold').setBackground('#e6f3ff');
  ...
  console.log(`Created Varsity/JV proposal with ${femaleRankings.length} female and ${maleRankings.length} male swimmers`);
}
...
function processTryoutRankings_(data) {
  ...
  if (eventColumns.length === 0) {
    throw new Error('No required event columns found. Expected: ' + requiredEvents.join(', '));
  }

  console.log(`Found ${eventColumns.length} of ${requiredEvents.length} required events:`, 
...
function calculateTryoutRankings_(swimmers, eventColumns, gender) {
  const rankings = [];
  const totalSwimmers = swimmers.filter(swimmer => swimmer[0] && swimmer[0].toString().trim()).length;
  ...
  eventColumns.forEach(event => {
    ...
    swimmerTimes.forEach((entry, index) => {
  ...
  swimmers.forEach((swimmer, swimmerIndex) => {
    ...
    const avgRank = swimmerRanks.length > 0
      ? Math.round((swimmerRanks.reduce((sum, rank) => sum + rank, 0) / swimmerRanks.length) * 100) / 100
      : null;
    ...
    eventColumns.forEach(event => {
      rankingData.eventRanks[event.name] = eventRankings[event.name][swimmerIndex] || null;
      const timeValue = swimmer[event.index];
      rankingData.eventTimes[event.name] = (timeValue && timeValue.toString().trim()) || null;
    });
  ...
  ui.createMenu('Coach Tools')
    ...
    .addSubMenu(
      ui
        ...
        .addItem(
          '🏆 Generate Varsity/JV Squads',
          'generateVarsityJVSquads'
        )
    )
    .addSubMenu(
      ui
